  ...
  try {
    ...
    if (isLootBoxCategory) {
      ...
      if (masterBox) {
        ...
        if (itemsEnabled) {
          const rarityLines = [
            `<:Common:1540257440971366400> **Common**: \`${masterBox.chance_common}%\``,
            `<:Uncommon:1540257439629312010> **Uncommon**: \`${masterBox.chance_uncommon}%\``,
            `<:Rare:1540257438274428928> **Rare**: \`${masterBox.chance_rare}%\``,
            `<:Epic:1540257436890566666> **Epic**: \`${masterBox.chance_epic}%\``,
            `<:Legendary:1540257435560841236> **Legendary**: \`${masterBox.chance_legendary}%\``
          ].join('\n');
    ...
    const rarityText = RARITY_DISPLAY_MAP[item.rarity] || '<:Common:1540257440971366400> Common';
  ...
  try {
    ...
    if (action === 'open') {
      ...
      for (const itemEntry of itemCounts.values()) {
        const rarityBadge = RARITY_EMOJIS[(itemEntry.rarity || 'common').toLowerCase()] || '<:Common:1540257440971366400>';
